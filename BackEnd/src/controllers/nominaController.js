...
const NominaFecha = require("../models/nominaFecha");
...
const obtenerNominasFecha = async(req, res, next) => {

    try {
        const nominasFecha = await firestore.collection('/Gobierno Autonomo Descentralizado Parroquial/Uyumbicho/NominaPago').doc("NominaPago").listCollections();
        const data = await nominasFecha.get();

        


        console.log(data.map(col => col.id));
        const nominasFechasArray = [];
        
        if (data.empty) {
            res.status(404).send('No se encontraron fechas');
        } else {
            data.forEach(collection => {
                const nominaFecha = new NominaFecha(
                    "doc.id",
                    "doc.data().anho",
                   " doc.data().mes"
                   
                );
                res.json(collection.data().id);
                nominasFechasArray.push(nominaFecha);
            });
            res.json(doc.data());
        }
    } catch (error) {
